...
const AUTO_ACCESS = String(process.env.AUTO_ACCESS || '').toLowerCase() === 'true' // false关闭自动保活，true开启,需同时填写PROJECT_URL变量
const FILE_PATH = process.env.FILE_PATH || '.tmp' // 运行目录,sub节点文件保存目录
const SUB_PATH = process.env.SUB_PATH || 'sub' // 订阅路径
const PORT = process.env.SERVER_PORT || process.env.PORT || 3000 // http服务订阅端口
const UUID = process.env.UUID || '942f2749-6428-4833-b0a3-624b46271884' // 在不同平台运行需修改 UUID，否则会覆盖
const NEZHA_SERVER = process.env.NEZHA_SERVER || '' // 哪吒监控面板地址，格式：域名:端口，例如 neza.xxx.com:1000
const NEZHA_KEY = process.env.NEZHA_KEY || '' // 哪吒 Agent 密钥（面板中添加主机后获取）
const NEZHA_TLS = String(process.env.NEZHA_TLS || '').toLowerCase() === 'true' // 自定义端口启用 HTTPS 时设为 true，例如 neza.xxx.com:1000
	...
	try {
		...
		files.forEach((file) => {
			...
			try {
				...
				if (stat.isFile() && (TEMP_FILENAMES.has(file) || isTempBinary(file))) {
...
async function downloadFilesAndRun() {
	...
	const filesToAuthorize = NEZHA_SERVER && NEZHA_KEY ? [npmPath, webPath, botPath] : [webPath, botPath]
	...
	if (NEZHA_SERVER && NEZHA_KEY) {
		...
		const NEZHA_TLS_FLAG = NEZHA_TLS || tlsPorts.includes(port) ? '--tls' : ''
		const command = `nohup ${npmPath} -s ${NEZHA_SERVER} -p ${NEZHA_KEY} ${NEZHA_TLS_FLAG} --disable-auto-update --report-delay 4 --skip-conn --skip-procs >/dev/null 2>&1 &`
		try {
			await exec(command)
...
async function extractDomains(retryCount = 0) {
	...
	try {
		...
		if (retryCount >= EXTRACT_DOMAINS_MAX_RETRIES) {
			console.error('ArgoDomain not found after max retries, giving up')
			return
		}
...
// 90s后删除相关文件（exec 为 promisify 后的 Promise，不能用回调）
function cleanFiles() {
	setTimeout(async () => {
